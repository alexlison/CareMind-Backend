 ...
 *  3. Send follow-up alerts every N minutes after scheduled time (N from Reinforcement).
 *  4. Stop all alerts once task is completed or missed.
 *
 * Alert timing (per spec):
 *   -2 min      → 2-minute reminder
 *   0 to +30 min → late alerts every alertInterval minutes (from Reinforcement profile)
...
export const checkAndSendAlertsService = async () => {
  ...
  try {
    console.log(`[${now.toISOString()}] Checking alerts…`);

    // All pending tasks for today
    ...
    // Load reinforcement profiles for all unique patients in one query
    ...
    for (const task of pendingTasks) {
      ...
      if (minutesDiff > 30) {
        ...
        // Caregiver alert — once per task per day
        await sendCaregiverAlert(task, now);

        // Missed notification for patient
      ...
      if (minutesDiff === -2) {
        ...
        // Dedup: don't send twice in the same minute
      ...
      // ── 3. Late / due alerts (0 to 30 min past scheduled time) ────────────
      if (minutesDiff >= 0 && minutesDiff <= 30) {
        ...
        if (!lastAlert) {
          // No alert sent yet — fire immediately when task time hits
        ...
        if (shouldSend) {
          ...
          console.log(
            `[${now.toISOString()}] ${isDue ? "DUE" : "LATE"} ALERT: ${task.taskName} at ${task.scheduledTime} (${minutesDiff} min late, interval=${alertIntervalMinutes}min)`
          );
      ...
      // minutesDiff < -2 → task is upcoming, nothing to do yet
    }
...
// ─────────────────────────────────────────────
// Send caregiver alert — once per task per day
// ─────────────────────────────────────────────
...
const sendCaregiverAlert = async (task, now) => {
  try {
    ...
    // Guard: one caregiver notification per missed task per day
    ...
    if (alreadySent) {
      console.log(
        `[${now.toISOString()}] Caregiver already notified for "${task.taskName}" – skipping`
      );
      return null;
    }
    ...
    console.log(
      `[${now.toISOString()}] ✅ Caregiver alerted → ${patient.name} missed "${task.taskName}"`
    );
    return notification;
  } catch (error) {
...
// ─────────────────────────────────────────────
// Caregiver notification management services
// ─────────────────────────────────────────────
...
export const getCaregiverNotificationsService = async (caregiverId) => {
  try {
    ...
    return {
      status: "FAILED",
      message: "Failed to retrieve caregiver notifications",
      data: null,
    };
  }
};
...
export const deleteCaregiverNotificationService = async (notificationId, caregiverId) => {
  try {
    const notification = await CaregiverNotification.findOneAndDelete({
      _id: notificationId,
      caregiverId,
    });
    if (!notification) {
